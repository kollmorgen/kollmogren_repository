...
void TimeDispatch(void)
{
	...
	{
		if (row->state & TIMER_ARMED) /* if row is active 如果被激活的话 */
		{
			if (row->val <= real_total_sleep_time) /* to be trigged */
			{
				if (!row->interval) /* if simply outdated */
				{
					row->state = TIMER_TRIG; /* ask for trig定时器触发状态 */
				}
				else /* or period have expired期限已经到期 */
				{
					/* set val as interval, with 32 bit overrun correction, 将val设置为间隔，具有32位溢出校正，* /
					/* modulo for 64 bit not available on all platforms   64位模不是在所有平台上都可用*  */
	...
	/* Set timer to soonest occurence 最快发生*/
	...
	for(i=0, row = timers; i<=last_timer_raw; i++, row++)
	{
		...
		{
			row->state &= ~TIMER_TRIG; /* reset trig state 重置触发状态(will be free if not periodic) */
			if(row->callback)
